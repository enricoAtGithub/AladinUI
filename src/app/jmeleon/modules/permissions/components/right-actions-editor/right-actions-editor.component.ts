...
export class RightActionsEditorComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.entryId) {
      this.facade.requestActionTreeFromBackend(this.mainId);
  ...
  nodeSelect(event) {
    ...
    this.facade.addActionToRight(this.mainId, event.node, this.selectedActions);

  }
  nodeUnselect(event) {
    // console.log('unselected node: ', event.node);
    // this.updatingSelectedAreaActive = true;
    this.facade.removeActionFromRight(this.mainId, event.node, this.selectedActions);
